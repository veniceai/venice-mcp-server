...
const fail = (text, structured) => ({
    content: [{ type: 'text', text }],
    isError: true,
    ...(structured ? { structuredContent: structured } : {}),
});
function decodeEnhancedPrompt(headers) {
...
function needsConsentDetails(err) {
    if (!(err instanceof VeniceUpstreamError) || err.status !== 409)
        return undefined;
    const body = err.body;
...
export function buildTools(client, cfg) {
    ...
    const tools = [
        ...
        // ========================================================================
        {
            ...
            handler: async (args) => {
                try {
                    const { data: resp, headers } = await client.postWithMetadata('/v1/image/generate', {
                        ...args,
                    ...
                    const images = resp.images?.filter((image) => typeof image === 'string' && image.length > 0) ?? [];
                    if (images.length > 0) {
                        ...
                        if (enhancedPrompt)
                            content.push({ type: 'text', text: `Enhanced prompt: ${enhancedPrompt}` });
                        return {
                            content,
                    ...
                    if (dataContent.length > 0) {
                        if (enhancedPrompt)
                            dataContent.push({ type: 'text', text: `Enhanced prompt: ${enhancedPrompt}` });
                        return {
                            content: dataContent,
                            structuredContent: {
                ...
                try {
                    ...
                    return {
                        content: [
                            ...
                            ...(enhancedPrompt ? [{ type: 'text', text: `Enhanced prompt: ${enhancedPrompt}` }] : []),
                        ],
            ...
            handler: async (args) => {
                try {
                    ...
                    return ok(`Queued: queue_id=${id}, model=${resp.model}\n` +
                        `Poll with venice_video_status(${pollArgs})`, { queue_id: id, model: resp.model, download_url: resp.download_url });
                }
                catch (err) {
                    ...
                    if (consent) {
                        ...
                        return fail(`Seedance consent is required for: ${roles}.\n\nPolicy text:\n${policy}\n\n` +
        ...
        },
        {
            ...
            inputSchema: {
                ...
                download_url: z.string().url().optional().describe('Queue-time download_url from venice_video_generate. Required for VPS / Grok Imagine Private models: retrieve returns COMPLETED without a URL.'),
                delete_media_on_completion: z.boolean().optional(),
            },
            handler: async (args) => {
                try {
                    const { download_url: queueDownloadUrl, ...retrieveArgs } = args;
                    const response = await client.postMixed('/v1/video/retrieve', {
                        ...retrieveArgs,
                    ...
                    const cleanupAfterSuccess = async (successNote) => {
                    ...
                    if (response.kind === 'binary') {
                        ...
                        const { deleted, cleanupNote } = await cleanupAfterSuccess('Server-side media was deleted after the MP4 was buffered.');
                        return {
                            content: [
                                {
                                    type: 'resource',
                    ...
                    if (resp.status === 'COMPLETED') {
                        if (!url) {
                            return fail('Video completed but Venice returned neither a video/mp4 body nor a download_url.', { status: 'COMPLETED' });
                        }
                        const { deleted, cleanupNote } = await cleanupAfterSuccess('Server-side media was deleted after the download URL was captured.');
                        return {
                            content: [
                    ...
                    if (err instanceof VeniceResponseTooLargeError) {
                        return fail(`Completed video exceeds the configured ${err.maxBytes}-byte MCP response limit. ` +
        ...
        },
        {
            ...
            description: `Fetch step-1 top-up requirements for a wallet. The API accepts an empty POST; the address is validated locally for the caller's intended wallet. Signing and payment-header submission happen OUTSIDE this MCP server.`,
            inputSchema: {
                wallet_address: z.string().regex(/^0x[a-fA-F0-9]{40}$/),
            },
            handler: async () => {
